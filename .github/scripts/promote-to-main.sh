...
assert_sequential_version "$files" "$PACKAGE" "$VERSION"
...
dev_build="$(jq -r '.attrs.build' <<< "$dev_file")"
main_build="$(find_highest_build "$files" "$VERSION" main |
  jq -r '.attrs.build // -1')"

if [ "$dev_build" -le "$main_build" ]; then
  exit_failure "$PACKAGE: dev build $dev_build must exceed $main_build"
fi
